...
namespace H.Styles.Default
{
    static class FontIcons
    {
        ...
        public static IEnumerable<Tuple<string, string, string, string>> GetValues()
        {
            foreach (var item in typeof(FontIcons).GetFields())
            {
                yield return Tuple.Create(item.Name, item.GetValue(null)?.ToString(), $"x:Static FontIcons.{item.Name}", $"FontIcons.{item.Name}");
            }
        }
    }
