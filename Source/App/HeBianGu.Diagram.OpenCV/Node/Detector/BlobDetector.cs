...
namespace HeBianGu.Diagram.OpenCV
{
    ...
    public class BlobDetector : OpenCVNodeData
    {
        ...
        public override IFlowableResult Invoke(Part previors, Node current)
        {
            ...
            var param = CopyTo();
            var circleDetector = SimpleBlobDetector.Create(param);
        ...
        void CopyFrom(SimpleBlobDetector.Params @params)
        {
            var ps = @params.GetType().GetProperties();
            var tps = GetType().GetProperties();

            foreach (var p in ps)
            {
                var f = tps.FirstOrDefault(x => x.Name == p.Name && x.PropertyType == p.PropertyType);
                if (f == null)
                    continue;
                if (!f.CanWrite)
                    continue;
        ...
        SimpleBlobDetector.Params CopyTo()
        {
            var @params = new SimpleBlobDetector.Params();
            var ps = @params.GetType().GetProperties();
            var tps = GetType().GetProperties();
