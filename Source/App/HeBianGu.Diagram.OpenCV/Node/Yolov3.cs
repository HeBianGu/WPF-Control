...
namespace HeBianGu.Diagram.OpenCV
{
    ...
    public class Yolov3 : OpenCVNodeData
    {
        ...
        {
            if (!File.Exists(this.WeightFilePath))
            {
        ...
        protected override IFlowableResult Refresh()
        {
            ...
            string[] lables = System.IO.File.ReadAllLines(this.NameFilePath).ToArray();

            var net = CvDnn.ReadNetFromDarknet(this.CfgFilePath, this.WeightFilePath);
            //读入模型和设置
            ...
            foreach (var i in indices)
            {
                ...
                var label = string.Format("{0} {1:0.0}%", lables[classIds[i]], probabilities[i] * 100);
