...
namespace H.Controls.Diagram.Extension
{
    public class FlowableLinkData : TextLinkData, IFlowableLink
    {
        ...
        {
            Thread.Sleep(AppSetting.Instance.FlowSleepMillisecondsTimeout);
            if (AppSetting.Instance.UseMock)
            {
        ...
        public virtual async Task<IFlowableResult> TryInvokeAsync(Part previors, Link current)
        {
            ...
            catch (Exception ex)
            {
                ...
                Logger.Instance?.Info($"执行错误<{this.GetType().Name}>:{this.Text} {this.Message}");
                Logger.Instance?.Error($"执行错误<{this.GetType().Name}>:{this.Text} {this.Message}");
