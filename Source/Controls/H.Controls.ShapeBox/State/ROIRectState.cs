...
public class ROIRectState : PreviewShapeStateBase
{
    ...
    {
        if (this._rOIRectShape is IPreviewShape preview)
            yield return preview;
        yield return _crossPreviewShape;
    }
    ...
    {
        var cursor = this.GetCursor(e.GetPosition(this.GetElementView()));
        e.Cursor = cursor;
    }
    ...
    {
        yield return this._rOIRectShape;
    }
...
public interface IRectCropable
{
    Rect Rect { get; set; }
    //void OnRectChanged(Rect oRect, Rect nRect);
}

[Icon(FontIcons.Crop)]
[Display(Name = "截取图片")]
public class RectCropableState : ROIRectState
{
    private readonly IRectCropable _rectCropable;
    public RectCropableState(IRectCropable rectCropable)
    {
    ...
    public override void Enter()
    {
        this.ROIRectShape.Rect = this._rectCropable.Rect.IsZoreOrEmpty() ? System.Windows.Rect.Empty : this._rectCropable.Rect;
        base.Enter();
    }
}
