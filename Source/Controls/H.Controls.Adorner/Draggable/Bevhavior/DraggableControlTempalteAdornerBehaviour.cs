...
{
    public class DraggableControlTempalteAdornerBehaviour : DraggableAdornerBehaviorBase
    {
        ...
        public static readonly DependencyProperty IsUseProperty =
            DependencyProperty.RegisterAttached("IsUse", typeof(bool), typeof(DraggableControlTempalteAdornerBehaviour), new PropertyMetadata(default(bool), OnIsUseChanged));
        ...
        private static void RefreshUse(DependencyObject d, bool use)
        {
            ...
            IEnumerable<Behavior> finds = bevaviors.Where(x => x.GetType() == typeof(DraggableControlTempalteAdornerBehaviour));
            foreach (Behavior item in finds.ToList())
            {
                bevaviors.Remove(item);
            }
            if (use)
                bevaviors.Add(new DraggableControlTempalteAdornerBehaviour());
        }
    }
}
