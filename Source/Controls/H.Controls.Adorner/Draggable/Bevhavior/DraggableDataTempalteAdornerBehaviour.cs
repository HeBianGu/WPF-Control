...
{
    public class DraggableDataTempalteAdornerBehaviour : DraggableAdornerBehaviorBase
    {
        ...
        public static readonly DependencyProperty IsUseProperty =
            DependencyProperty.RegisterAttached("IsUse", typeof(bool), typeof(DraggableDataTempalteAdornerBehaviour), new PropertyMetadata(default(bool), OnIsUseChanged));
        ...
        private static void RefreshUse(DependencyObject d, bool use)
        {
            ...
            IEnumerable<Behavior> finds = bevaviors.Where(x => x.GetType() == typeof(DraggableDataTempalteAdornerBehaviour));
            foreach (Behavior item in finds.ToList())
            {
                bevaviors.Remove(item);
            }
            if (use)
                bevaviors.Add(new DraggableDataTempalteAdornerBehaviour());
        }
    }
}
