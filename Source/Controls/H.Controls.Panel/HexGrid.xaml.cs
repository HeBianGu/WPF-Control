...
public class HexGrid : System.Windows.Controls.Panel
{
    ...
    protected override Size MeasureOverride(Size availableSize)
    {
        ...
        if (Double.IsInfinity(w) || Double.IsInfinity(h))
        {
            ...
            return this.Orientation == Orientation.Horizontal
                ? new Size(w * (this.ColumnCount * 3 + 1) / 4, h * (this.RowCount * 2 + 1) / 2)
                : new Size(w * (this.ColumnCount * 2 + 1) / 2, h * (this.RowCount * 3 + 1) / 4);
        }
    ...
    private Size GetPossibleSize(Size gridSize)
    {
        ...
        return this.Orientation == Orientation.Horizontal
            ? GetPossibleSizeHorizontal(gridSize, first, last)
            : GetPossibleSizeVertical(gridSize, first, last);
    }
