...
public static class GridAreaAttach
{
        ...
        foreach (FrameworkElement child in grid.Children)
        {
            ...
            if (isFixed)
            {
                ...
                for (int i = row; i < row + rowSpan; i++)
                    ...
                    {
                        if (columnCount <= j || rowCount <= i) continue; area[i, j] = true;
                    }
            }
            else
            {
    ...
    private static void InitializeColumnDefinition(Grid grid, string param)
    {
        ...
        foreach (string item in list)
        {
            ...
            if (def.Max != null) columnDefinition.MaxWidth = def.Max.Value; if (def.Min != null) columnDefinition.MinWidth = def.Min.Value; grid.ColumnDefinitions.Add(columnDefinition);
        }
    }
    ...
    private static void InitializeRowDefinition(Grid grid, string param)
    {
        ...
        foreach (string item in list)
        {
            ...
            if (def.Max != null) rowDefinition.MaxHeight = def.Max.Value; if (def.Min != null) rowDefinition.MinHeight = def.Min.Value; grid.RowDefinitions.Add(rowDefinition);
        }
    }
    ...
    private static AreaDefinition GetAreaNameRegion(FrameworkElement element)
    {
        ...
        if (grid == null || name == null) return null; IList<NamedAreaDefinition> areaList = GetAreaDefinitions(grid);
        if (areaList == null) return null;
        NamedAreaDefinition area = areaList.FirstOrDefault(o => o.Name == name);
        return area == null ? null : new AreaDefinition(area.Row, area.Column, area.RowSpan, area.ColumnSpan);
    }
    ...
    private static AreaDefinition GetAreaRegion(FrameworkElement element)
    {
        ...
        return list.Count() != 4 ? null : new AreaDefinition(list[0], list[1], list[2], list[3]);
    }
}
