...
public class CoverPanel : PanelBase
{
                ...
                if (k.Parameter is UIElement element)
                {
                    ...
                    if (index >= 0)
                    {
                        this.StartIndex = index > (this.Children.Count - 1) / 2
                            ? index - (this.Children.Count - 1) / 2
                            : this.Children.Count - ((this.Children.Count - 1) / 2 - index);
                    }
                }
            };
        }
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        for (int i = 0; i < children.Count; i++)
        {
            ...
            {
                elment.RenderTransformOrigin = this.VerticalContentAlignment == VerticalAlignment.Top ? new Point(0.5, 0.0) : new Point(0.5, 0.5);
            }

            SetZIndex(elment, children.Count - (int)centerValue);
