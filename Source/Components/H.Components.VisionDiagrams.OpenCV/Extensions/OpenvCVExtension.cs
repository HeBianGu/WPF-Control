...
public static class OpenvCVExtension
{
    ...
    public static RotatedRectFlipResult GetForegroundRectFlipResult(this IEnumerable<Point2f> ps, double flipSensitivity = 0.01)
    {
        var points = ps.ToList();
        var rr = Cv2.MinAreaRect(points);
        float angle = rr.GetCorrectAngle();
        // 基于 RotatedRect 的左右/上下判定（相对于旋转矩形的局部坐标系）
        // 计算局部坐标轴：X 轴沿宽度方向，Y 轴沿高度方向
        double theta = angle * Math.PI / 180.0; // OpenCV 角度为度
        var axisX = new Point2f((float)Math.Cos(theta), (float)Math.Sin(theta));       // 局部 X（宽度方向）
        var axisY = new Point2f((float)-Math.Sin(theta), (float)Math.Cos(theta));    // 局部 Y（高度方向），为 X 逆时针旋转 90°

        int left = 0, right = 0, top = 0, bottom = 0;
        for (int i = 0; i < points.Count; i++)
        {
            var vx = points[i].X - rr.Center.X;
            var vy = points[i].Y - rr.Center.Y;

            // 投影到局部坐标轴
            double projX = vx * axisX.X + vy * axisX.Y; // 相对于局部 X（宽度）的左右
            double projY = vx * axisY.X + vy * axisY.Y; // 相对于局部 Y（高度）的上下
            if (projX < 0) left++; else right++;
            if (projY < 0) top++; else bottom++;
        }

        double hTotal = Math.Max(1, left + right);
        double vTotal = Math.Max(1, top + bottom);
        double hDiff = Math.Abs(right - left) / hTotal;
        double vDiff = Math.Abs(bottom - top) / vTotal;

        bool flipH = right > left && hDiff >= flipSensitivity;
        bool flipV = bottom > top && vDiff >= flipSensitivity;
        // 聚合到统一结果属性
        return new RotatedRectFlipResult(rr, flipH, flipV);
    }
