// Copyright (c) HeBianGu Authors. All Rights Reserved. 
// Author: HeBianGu 
// Github: https://github.com/HeBianGu/WPF-Control 
// Document: https://hebiangu.github.io/WPF-Control-Docs  
// QQ:908293466 Group:971261058 
// bilibili: https://space.bilibili.com/370266611 
// Licensed under the MIT License (the "License")

namespace H.Components.VisionDiagrams.OpenCV.NodeDatas.Basic;
[Icon(FontIcons.Color)]
[Display(Name = "颜色转换", GroupName = "基础函数", Description = "设置图片颜色", Order = 2)]
public class CvtColorNodeData : OpenCVNodeDataBase, IPreprocessingGroupableNodeData
{
    private ColorConversionCodes _colorConversionCode = ColorConversionCodes.BGR2GRAY;

    [DefaultValue(ColorConversionCodes.BGR2GRAY)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "转换规则", GroupName = VisionTabNames.RunParameters, Description = "选择输入图像的颜色空间转换规则")]
    public ColorConversionCodes ColorConversionCode
    {
        get { return _colorConversionCode; }
        set
        {
            _colorConversionCode = value;
            RaisePropertyChanged();
            this.Invoke();
        }
    }

    private int _dstCn = 0;
    [DefaultValue(0)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "通道数", GroupName = VisionTabNames.RunParameters, Description = "设置颜色转换结果的目标通道数，使用 0 时自动推断")]
    public int DstCn
    {
        get { return _dstCn; }
        set
        {
            _dstCn = value;
            RaisePropertyChanged();
            this.Invoke();
        }
    }

    protected override FlowableResult<IMatImage> Invoke(Mat fromImage)
    {
        if (fromImage.Channels() == 1)
            return this.OK(fromImage.Clone());
        Mat mat = fromImage.CvtColor(this.ColorConversionCode, this.DstCn);
        return this.OK(mat);
    }

    public override void Clear()
    {
        base.Clear();
    }
}
