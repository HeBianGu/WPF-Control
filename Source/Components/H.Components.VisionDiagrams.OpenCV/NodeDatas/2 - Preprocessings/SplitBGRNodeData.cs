...
public class SplitBGRNodeData : OpenCVNodeDataBase, IPreprocessingGroupableNodeData
{
    private SplitSelectType _splitSelectType;
    [DefaultValue(SplitSelectType.B)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "选择通道", GroupName = VisionTabNames.RunParameters, Description = "选择从 BGR 图像中提取的颜色通道")]
    ...
    [DefaultValue(true)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "启用合并", GroupName = VisionTabNames.RunParameters, Description = "指定是否将提取通道重新合成为三通道图像")]
