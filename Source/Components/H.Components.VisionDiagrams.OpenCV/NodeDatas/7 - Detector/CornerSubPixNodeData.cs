...
{
    [Display(Name = "全部", GroupName = "筛选方式", Description = "保留检测到的全部角点")]
    All,
    [Display(Name = "卡尺筛选", GroupName = "筛选方式", Description = "仅保留位于卡尺区域内的角点")]
    Caliper
}
...
public abstract class PointDetectorNodeDataBase : OpenCVDetectorNodeDataBase, IDetectorGroupableNodeData
{
    ...
    [Display(Name = "角点结果", GroupName = VisionTabNames.ResultParameters, Description = "查找到的点数据")]
    ...
    [Display(Name = "角点筛选方式", GroupName = VisionTabNames.RunParameters, Description = "All:不过滤；Caliper:按卡尺区域过滤")]
...
public class CornerSubPixNodeData : PointDetectorNodeDataBase, IDetectorGroupableNodeData
{
    private int _maxCorners = 500;
    [DefaultValue(500)]
    [Range(1, 1000)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "最大角点数", GroupName = VisionTabNames.RunParameters, Description = "返回的最大角点数量")]
    ...
    [Range(0.003, 1.0)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "质量等级", GroupName = VisionTabNames.RunParameters, Description = "角点质量阈值，较大值更严格")]
    ...
    [Range(0.0, 500.0)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "最小距离", GroupName = VisionTabNames.RunParameters, Description = "两个角点之间的最小像素距离")]
    ...
    [Range(1, 50)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "Block 大小", GroupName = VisionTabNames.RunParameters, Description = "用于计算梯度协方差矩阵的邻域大小")]
    ...
    [DefaultValue(true)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "亚像素细化", GroupName = VisionTabNames.RunParameters, Description = "是否使用 CornerSubPix 对角点位置进行细化")]
    ...
    [Range(1, 50)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "SubPix 窗口大小", GroupName = VisionTabNames.RunParameters, Description = "CornerSubPix 搜索窗口大小的一维长度")]
