...
public class QRCodeNodeData : OpenCVDetectorNodeDataBase, IDetectorGroupableNodeData
{
    private string _qrCodeResult;
    [ReadOnly(true)]
    [Expressionable]
    [Tab(VisionTabNames.ResultParameters)]
    [Display(Name = "二维码结果", GroupName = VisionTabNames.ResultParameters, Description = "结果参数，此结果可应用再条件分支等作为判断参数")]
    ...
    [Display(Name = "绘制结果方式", GroupName = VisionTabNames.DisplayParameters, Description = "选择二维码定位边框和文本结果的绘制方式")]
