...
public class CannyNodeData : OpenCVNodeDataBase, IDetectorGroupableNodeData
{
    ...
    [Display(Name = "低阈值", GroupName = VisionTabNames.RunParameters, Description = "用于边缘连接的最小梯度值,低于此值的像素不会被考虑为边缘")]
    ...
    [Range(150.0, 200.0)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "高阈值", GroupName = VisionTabNames.RunParameters, Description = "确定强边缘的最小梯度值,高于此值的像素被确定为强边缘,通常设置为 threshold1 的 2-3 倍")]
    ...
    [DefaultValue(3)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "Sobel核大小", GroupName = VisionTabNames.RunParameters, Description = "用于计算图像梯度的 Sobel 算子的孔径大小,可选值: 3, 5, 7,较大的孔径可以检测到更模糊的边缘，但计算量更大")]
    ...
    [DefaultValue(false)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "梯度计算方式", GroupName = VisionTabNames.RunParameters, Description = "指定计算梯度幅值的方法,false (默认): 使用 L1 范数 (|dI/dx| + |dI/dy|)，计算更快,true: 使用更精确的 L2 范数 (√[(dI/dx)² + (dI/dy)²])")]
