...
public class SVMNodeData : OpenCVNodeDataBase, IOtherGroupableNodeData
{
    ...
    protected override FlowableResult<IMatImage> Invoke(Mat fromImage)
    {
        ...
        using (Mat pointsPlot = Mat.Zeros(300, 300, MatType.CV_8UC3))
        {
            ...
            //this.Image?.Dispose();
            //this.Image = pointsPlot;
            //this.ResultImageSource = this.ToImageSource(pointsPlot);
            Thread.Sleep(1000);
        }

        // Train
        Mat dataMat = Mat.FromPixelData(points.Length, 2, MatType.CV_32FC1, points);
        Mat resMat = Mat.FromPixelData(responses.Length, 1, MatType.CV_32SC1, responses);
        ...
        for (int x = 0; x < 300; x++)
        {
            for (int y = 0; y < 300; y++)
            {
                ...
                Mat sampleMat = Mat.FromPixelData(1, 2, MatType.CV_32FC1, sample);
                int ret = (int)svm.Predict(sampleMat);
                OpenCvSharp.Rect plotRect = new OpenCvSharp.Rect(x, 300 - y, 1, 1);
                if (ret == 1)
