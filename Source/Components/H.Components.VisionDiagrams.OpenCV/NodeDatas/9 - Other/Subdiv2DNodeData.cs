...
public class Subdiv2DNodeData : OpenCVNodeDataBase, IOtherGroupableNodeData
{
    ...
    [Display(Name = "单元类型", GroupName = VisionTabNames.RunParameters, Description = "定义了平面细分的输出单元类型")]
    ...
    [DefaultValue(600)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "单元大小", GroupName = VisionTabNames.RunParameters, Description = "定义了细分后单元的目标大小")]
    public int Size
    {
        get { return _size; }
        set
    ...
    protected override FlowableResult<IMatImage> Invoke(Mat fromImage)
    {
        ...
        {
            img.Circle((OpenCvSharp.Point)p, 4, Scalar.Red, -1);
        }

        // Initializes Subdiv2D
        using OpenCvSharp.Subdiv2D subdiv = new OpenCvSharp.Subdiv2D();
        subdiv.InitDelaunay(new OpenCvSharp.Rect(0, 0, Size, Size));
        ...
        {  // Draws voronoi diagram
            ...
            foreach (Point2f[] list in facetList)
            {
                ...
                {
                    vonoroi.Line((OpenCvSharp.Point)before, (OpenCvSharp.Point)p, new Scalar(64, 255, 128), 1);
                    before = p;
                }
            }
...

        {
            ...
            {
                OpenCvSharp.Point p1 = new OpenCvSharp.Point(edge.Item0, edge.Item1);
                OpenCvSharp.Point p2 = new OpenCvSharp.Point(edge.Item2, edge.Item3);
