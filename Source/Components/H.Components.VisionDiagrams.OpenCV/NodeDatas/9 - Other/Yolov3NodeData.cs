...
public class Yolov3NodeData : OpenCVNodeDataBase, IOtherGroupableNodeData
{
    ...
    [Display(Name = "配置文件路径", GroupName = VisionTabNames.RunParameters, Description = "设置 YOLOv3 网络结构配置文件路径")]
    ...
    [Required]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "权重文件路径", GroupName = VisionTabNames.RunParameters, Description = "设置 YOLOv3 训练权重文件路径")]
    [PropertyItem(typeof(OpenFileDialogPropertyItem))]
    ...
    [Required]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "标签文件路径", GroupName = VisionTabNames.RunParameters, Description = "设置与模型输出类别对应的标签文件路径")]
    [PropertyItem(typeof(OpenFileDialogPropertyItem))]
    ...
    [Range(0.0f, 1.0f)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "置信度阈值", GroupName = VisionTabNames.RunParameters, Description = "置信度分数阈值，低于此值的框会被直接过滤")]
    ...
    [Range(0.0f, 1.0f)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "NMS重叠阈值", GroupName = VisionTabNames.RunParameters, Description = "NMS重叠阈值（IoU阈值），大于此值的框会被视为冗余而被抑制")]
    ...
    [Display(Name = "目标数量", GroupName = VisionTabNames.ResultParameters, Description = "结果参数，此结果可应用再条件分支等作为判断参数")]
    ...
    protected override FlowableResult<IMatImage> Invoke(Mat fromImage)
    {
        ...
        Mat org = fromImage.Clone();
        //生成blob, 块尺寸可以是320/416/608
        Mat blob = CvDnn.BlobFromImage(org, 1.0 / 255, new OpenCvSharp.Size(320, 320), new Scalar(), true, false);
        // 输入数据
        net.SetInput(blob);
        //获得输出层名
        ...
        this.ResultShapes = rects.Select(x => new RectShape(x.Item3) { Title = x.Item1 }).OfType<IShape>().ToObservable();
        this.MatchingCountResult = boxes.Count;
        Controls.Diagram.Presenter.NodeDatas.Base.IResultPresenter resultPresenter = rects.ToResultPresenter();
