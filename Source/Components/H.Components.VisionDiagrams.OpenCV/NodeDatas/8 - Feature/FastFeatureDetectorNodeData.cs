...
[Display(Name = "FAST特征提取", GroupName = "特征提取", Description = "使用 FAST 算法快速检测图像角点", Order = 0)]
public class FastFeatureDetectorNodeData : FeatureOpenCVNodeDataBase
{
    private bool _nonmaxSupression = true;
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "非极大值抑制", GroupName = VisionTabNames.RunParameters, Description = "指定是否抑制相邻区域内响应较弱的重复角点")]
    ...
    [Range(0, 500)]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "检测阈值", GroupName = VisionTabNames.RunParameters, Description = "设置中心像素与邻域像素的最小亮度差")]
