...
public class FeaturePointTemplateMatchNodeData : MatchingNodeData<IMatImage>, ITemplateMatchingGroupableNodeData, IRectCropable, IBase64MatchingNodeData, IOpenCVNodeData
{
    private int _goodMatchesToKeep = 100;
    [DefaultValue(100)]
    [PropertyItem(typeof(Int32SliderTextPropertyItem))]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "保留匹配点数", GroupName = VisionTabNames.RunParameters, Description = "设置按距离排序后保留的最佳特征匹配点数量", Order = 0)]
    [Range(4, 100)]
    ...
    [PropertyItem(typeof(Int32SliderTextPropertyItem))]
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "最少匹配点数", GroupName = VisionTabNames.RunParameters, Description = "设置判定特征模板匹配成功所需的最少点数", Order = 1)]
    [Range(4, 100)]
    ...
    [Tab(VisionTabNames.RunParameters)]
    [Display(Name = "特征模板数据", GroupName = VisionTabNames.RunParameters, Description = "设置模板的关键点和特征描述数据", Order = 3)]
...
public class KeyPointShapeViewPropertyItem : ShapeViewPropertyItem
{
    ...
    [Display(Name = "模板设置", GroupName = CommandGroupNames.QuickAccessToolbar, Description = "打开特征点模板配置界面")]
    public DisplayCommand ShowTemplateManagerCommand => new DisplayCommand(async x =>
    {
            if (this.Obj is FeaturePointTemplateMatchNodeData nodeData)
        {
    ...
    [Display(Name = "删除模板", GroupName = CommandGroupNames.QuickAccessToolbar, Description = "删除当前保存的特征点模板")]
