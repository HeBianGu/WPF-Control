...
public abstract class SrcFilesVisionNodeData<T> : StartVisionNodeData<T>, ISrcFilesVisionNodeData<T>, ISrcFilesNodeData where T : class, IVisionImage
{
    ...
    /// 获取或设置是否使用所有图像
    /// </summary>
    public SrcFileLoopMode SrcFileLoopMode
    {
        get { return _SrcFileLoopMode; }
        set
        {
            _SrcFileLoopMode = value;
            RaisePropertyChanged();
        }
    }
    ...
    private SrcFilePathData _srcFilePath;
    [Expressionable]
    [Browsable(false)]
    ...
    private ObservableCollection<SrcFilePathData> _srcFilePaths = new ObservableCollection<SrcFilePathData>();
    public ObservableCollection<SrcFilePathData> SrcFilePaths
    {
    ...
    protected virtual void AddFiles()
    {
        IocMessage.IOFolderDialog.ShowOpenFolderAction(x =>
        {
            ...
            this.SrcFilePaths.AddRange(images.Select(x => new SrcFilePathData() { SrcFilePath = x }));
    ...
    protected virtual void AddFile()
    {
        IocMessage.IOFileDialog.ShowOpenImageFiles(x =>
        {
            ...
            {
                this.SrcFilePaths.Add(new SrcFilePathData() { SrcFilePath = item });
            }
            if (this.SrcFilePaths.Count == 0)
                this.SrcFilePath = this.SrcFilePaths.FirstOrDefault();
        });
    ...
    public RelayCommand DeleteImageDataCommand => new RelayCommand(async x =>
    {
        await IocMessage.Dialog.ShowDeleteDialog(x =>
        {
            ...
            var find = this.SrcFilePaths.ElementAtOrDefault(index);
    ...
    public new virtual bool IsValid(out string message)
    {
